...
G4int G4XrayGrazingAngleScattering::GetVerbosity() const
{
  return theVerbosity;
}

void G4XrayGrazingAngleScattering::SetVerbosity(G4int verbosity)
{
...
G4VParticleChange *G4XrayGrazingAngleScattering::PostStepDoIt(const G4Track &aTrack, const G4Step &aStep)
{
  ...
  if (theVerbosity >= all_details)
  {
    ...
    G4cout << "postStep information: position=" << pPostStepPoint->GetPosition()
           ...
           << " physical volume=" << postPhysicalVolume->GetName() << G4endl;
  }
  ...
  const G4double reflectivity = xsurface->reflect(photonEnergy, pre_n, post_n,
...
G4bool G4XrayGrazingAngleScattering::IsApplicable(const G4ParticleDefinition &aParticle)
{
  return (&aParticle == G4Gamma::Gamma());
}
                                                          ...
                                                          G4double &sReflectivity, G4double &pReflectivity)
{
  ...
  if (grazingAngle < 0 and grazingAngle > M_PI_2)
  {
    ...
    G4Exception("G4XrayGrazingAngleScattering::ReflectivityFromVacuum",
                "0",
                EventMustBeAborted,
                msg.str().c_str());
  }
                                                          ...
                                                          G4double &unpolarizedReflectivity)
{
  ...
  ReflectivityFromVacuum(grazingAngle, postRefractionIndex, sReflectivity, pReflectivity);
  unpolarizedReflectivity = 0.5 * (sReflectivity + pReflectivity);
}

// void Reflectance(G4double energy, G4double grazingAngle, G4complex preRefractionIndex, G4complex postRefractionIndex,
// 		 G4double & sReflectivity, G4double & pReflectivity) {
...
//   const G4bool s_total_reflection = abs(s_demon) != 0;
//   const G4complex s_amplitude_r = (s_total_reflection ? (c1n12 - c2)/s_denom : 1);
//   const G4complex s_amplitude_t = (s_total_reflection ? 2*c1n12/s_denom : 0);

//   const G4complex c2n12 = c2*n12;
//   const G4complex p_denom = c2n12+c1;
//   const G4bool p_total_reflection = abs(p_demon) != 0;
//   const G4complex p_amplitude_r = (p_total_reflection ? (c2n12 - c1)/p_denom : 1);
//   const G4complex p_amplitude_t = (p_total_reflection ? 2*c1n12/p_denom : 0);
...
G4double G4XrayGrazingAngleScattering::SpecularFraction(G4double energy, G4double grazingAngle, G4double rmsRoughness, G4complex reflactionIndex)
{
  ...
  const G4complex exponent = std::pow(4 * pi * rmsRoughness / wavelength, 2) * std::sin(grazingAngle) * sqrt(std::pow(reflactionIndex, 2) - std::pow(std::cos(grazingAngle), 2));
...
G4double G4XrayGrazingAngleScattering::SpecularFraction(G4double energy, G4double grazingAngle, G4double rmsRoughness)
{
  ...
  const G4double wavelength = h_Planck * c_light / energy;
  const G4double exponent = std::pow(4 * pi * rmsRoughness * std::sin(grazingAngle) / wavelength, 2.);
